...
describe("ArtifactUploader", () => {
  ...
  it("abort when upload failed with non-5xx response", async () => {
    ...
    expect(uploadMock).toBeCalledWith(url, contentLength, "raw", fileContents, "art1", releaseId);
    expect(uploadMock).toBeCalledWith(url, contentLength, "raw", fileContents, "art2", releaseId);
